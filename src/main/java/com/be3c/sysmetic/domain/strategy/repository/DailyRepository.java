...
public interface DailyRepository extends JpaRepository<Daily, Long> {
    ...
    @Query("SELECT d FROM Daily d WHERE d.strategy.id = :strategyId " +
            "AND FUNCTION('DATE', d.date) > :startDate ORDER BY d.date ASC")
    ...
    @Query(value = "SELECT * FROM daily d WHERE d.strategy_id = :strategyId " +
            "AND DATE(d.date) < :startDate ORDER BY d.date DESC LIMIT 1", nativeQuery = true)
    ...
    Page<Daily> findAllByDateBetween(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate, @Param("pageable") Pageable pageable);

    // 일간분석데이터 목록 조회 - 특정 년월
    @Query(value = "SELECT * FROM daily d WHERE d.strategy_id = :strategyId " +
            "AND YEAR(d.date) = :year AND MONTH(d.date) = :month", nativeQuery = true)
